...
from remote_kernel_provider.container import ContainerLifecycleManager
...
class KubernetesLifecycleManager(ContainerLifecycleManager):
    """Kernel lifecycle management for Kubernetes kernels."""
    def __init__(self, kernel_manager, lifecycle_config):
        super(KubernetesLifecycleManager, self).__init__(kernel_manager, lifecycle_config)
    ...
    def launch_process(self, kernel_cmd, **kwargs):
        ...
        return super(KubernetesLifecycleManager, self).launch_process(kernel_cmd, **kwargs)
    ...
    def terminate_container_resources(self):
        ...
        if result:
            self.log.debug("KubernetesLifecycleManager.terminate_container_resources, pod: {}.{}, kernel ID: {} has "
                           "been terminated.".format(self.kernel_namespace, self.container_name, self.kernel_id))
            self.container_name = None
            result = None  # maintain jupyter contract
        else:
            self.log.warning("KubernetesLifecycleManager.terminate_container_resources, pod: {}.{}, kernel ID: {} has "
    ...
    def _determine_kernel_namespace(self, **kwargs):
        ...
        # get it now.
        service_account_name = KubernetesLifecycleManager._determine_kernel_service_account_name(**kwargs)
    ...
    def get_lifecycle_info(self):
        ...
        lifecycle_info = super(KubernetesLifecycleManager, self).get_lifecycle_info()
    ...
    def load_lifecycle_info(self, lifecycle_info):
        ...
        super(KubernetesLifecycleManager, self).load_lifecycle_info(lifecycle_info)
